...
LOKI_VERBOSE="" "${WI_RUN}/new-line-inspector" "${testdir}/file"
loki-assert-eq 0 $?

echo -n HI > "${testdir}/file1" # Must trigger NLI
LOKI_VERBOSE="" "${WI_RUN}/new-line-inspector" "${testdir}/file1"
loki-assert-neq 0 $?
...
for ext in h hh hpp hxx H hp; do
    ...
    LOKI_VERBOSE="" "${WI_RUN}/header-inspector" "${testdir}/test.${ext}"
    loki-assert-eq 0 $?
